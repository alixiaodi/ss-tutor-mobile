...
export default {
  ...
  methods: {
    getQuestionTypeLabel(type, index = this.currentQuestion) {
      if (!type) return "";
      const question = this.questions[index]; // 使用传入的索引获取对应题目
      ...
      if (oldQuestion !== newQuestion) {
        ...
        if (currentQuestion?.type === "blank" || currentQuestion?.type === "essay") {
          ...
          if (currentQuestion.type === "blank" && Array.isArray(answer)) {
            ...
            formattedAnswer = answer; // 简答题直接使用答案字符串
          }
    ...
    handleBack() {
      uni.showModal({
        title: "提示",
        content: "返回首页将不保存答题记录，是否确认返回？",
        success: (res) => {
          if (res.confirm) {
            // 打印需要清除的记录
            console.log("需要清除的答题记录:", {
              answerId: this.answerId,
